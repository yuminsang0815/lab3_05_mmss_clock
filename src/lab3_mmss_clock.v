...
module lab3_mmss_clock #(
  ...
  // 4 kHz 스캔 카운터 (자리당 1 kHz, 4자리 시분할 다중화)
  always @(posedge clk_50mhz or posedge rst_p) begin
    if (rst_p) begin
      scan_count  <= {SCAN_WIDTH{1'b0}};
      scan_select <= 2'd0;
  ...
  // 4자리 동적 스캔 출력 멀티플렉서 (Active-low COM)
  // 분/초 구분점: mo 자리(scan_select=1)에서 dp(seg_data[0]) 활성화
  always @* begin
    seg_com  = 8'b1111_1111;
    seg_data = 8'b0000_0000;
    case (scan_select)
      2'd0: begin
        seg_com  = 8'b1111_0111; // COM[3] 활성화 (분 10의 자리)
        seg_data = seg_mt;
      end
      2'd1: begin
        seg_com  = 8'b1111_1011; // COM[2] 활성화 (분 1의 자리 + 소수점 dp)
        seg_data = seg_mo | 8'b0000_0001;
      end
      2'd2: begin
        seg_com  = 8'b1111_1101; // COM[1] 활성화 (초 10의 자리)
        seg_data = seg_st;
      end
      default: begin
        seg_com  = 8'b1111_1110; // COM[0] 활성화 (초 1의 자리)
        seg_data = seg_so;
      end
    endcase
  end
